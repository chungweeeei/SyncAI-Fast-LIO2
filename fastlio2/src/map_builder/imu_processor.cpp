...
bool IMUProcessor::initialize(SyncPackage & package)
{
  ...
  m_last_imu = m_imu_cache.back();  // 存最後一筆 IMU，銜接下一幀 propagation
...
{
  // 每一幀都要把它重置成只裝這一幀需要的 IMU 資料
  m_imu_cache.clear();
  m_imu_cache.push_back(m_last_imu);  // 放上一幀最後一筆
  m_imu_cache.insert(
    m_imu_cache.end(), package.imus.begin(), package.imus.end());  // 接上這幀的 IMU
  ...
  // 這一幀掃描期間，機器人「移動軌跡」的時間快照(就是 IMU 的移動軌跡)
  m_poses_cache.clear();
  m_poses_cache.emplace_back(
    0.0, m_last_acc, m_last_gyro, m_kf->x().v, m_kf->x().t_wi, m_kf->x().r_wi);
  ...
  for (auto it_imu = m_imu_cache.begin(); it_imu < (m_imu_cache.end() - 1); it_imu++) {
    ...
    inp.acc = acc_val;
    inp.gyro = gyro_val;
    m_kf->predict(inp, dt, m_Q);  // 把 IESKF 的狀態往前推進dt

    m_last_gyro = gyro_val - m_kf->x().bg;
    m_last_acc = m_kf->x().r_wi * (acc_val - m_kf->x().ba) + m_kf->x().g;
    double offset = tail.time - cloud_time_begin;  // 這一刻相對掃描開始的時間
    m_poses_cache.emplace_back(
      offset, m_last_acc, m_last_gyro, m_kf->x().v, m_kf->x().t_wi,
      m_kf->x().r_wi);  // ← 拍快照存進 cache
  }
  ...
  // 現在有了一幀點雲中，IMU 的移動軌跡後利用這個 pose 的軌跡將 pointcloud 進行 undistorted
