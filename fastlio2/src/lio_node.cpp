...
struct NodeConfig
{
  ...
  // filter's gravity magnitude is fixed at 9.81 (ieskf.cpp).
...
public:
  ...
  {
    RCLCPP_INFO(this->get_logger(), "[LIONode][%s] LIO Node Started", __func__);
    loadParameters();

    // register imu subscriber
    m_imu_sub = this->create_subscription<sensor_msgs::msg::Imu>(
      m_node_config.imu_topic, 10, std::bind(&LIONode::imuCB, this, std::placeholders::_1));

    // register scan subscriber
    if (m_node_config.lidar_type == 1) {
      RCLCPP_INFO(
        this->get_logger(), "[LIONode][%s] Lidar input: sensor_msgs/PointCloud2 (%s)", __func__,
        m_node_config.lidar_topic.c_str());
      m_pc2_sub = this->create_subscription<sensor_msgs::msg::PointCloud2>(
        m_node_config.lidar_topic, 10, std::bind(&LIONode::pc2CB, this, std::placeholders::_1));
    } else {
      RCLCPP_INFO(
  ...
  void loadParameters()
  {
    ...
    if (!config) {
      RCLCPP_WARN(this->get_logger(), "[LIONode][%s] FAIL TO LOAD YAML FILE!", __func__);
      return;
    }

    RCLCPP_INFO(
      this->get_logger(), "[LIONode][%s] LOAD FROM YAML CONFIG PATH: %s", __func__,
      config_path.c_str());
  ...
  void imuCB(const sensor_msgs::msg::Imu::SharedPtr msg)
  {
    ...
    if (timestamp < m_state_data.last_imu_time) {
      RCLCPP_WARN(this->get_logger(), "[LIONode][%s] IMU Message is out of order", __func__);
  ...
  void pc2CB(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
  {
    ...
    double timestamp = Utils::getSec(msg->header);
    if (timestamp < m_state_data.last_lidar_time) {
      RCLCPP_WARN(this->get_logger(), "[LIONode][%s] Lidar Message is out of order", __func__);
      std::deque<std::pair<double, pcl::PointCloud<pcl::PointXYZINormal>::Ptr>>().swap(
        m_state_data.lidar_buffer);
    }
    m_state_data.lidar_buffer.emplace_back(timestamp, cloud);
  ...
  bool syncPackage()
  {
    ...
    // 一幀 lidar 並不是一收到就成處理，必須等到 IMU 「補齊」到掃描結束時刻才行。
    ...
    // 如果 IMU 資料還無法涵蓋一幀的 pointcloud 那就代表無法將該幀的 pointcloud 進行 undistorted
